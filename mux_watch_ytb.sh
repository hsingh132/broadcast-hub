...
#!/usr/bin/env bash
# Supervisor for the YouTube B pipeline.
...
: "${YT_INGEST:=rtmp://b.rtmp.youtube.com/live2}"                  # YouTube ingest base (Channel B)
: "${YT_KEY:=}"                                                    # Live key, required by consumer

# --- Local RTMP endpoints (Nginx-RTMP apps) ---
: "${OBS_OUT:=rtmp://localhost:1935/yt_producer_b}"                # where producer_obs_ytb publishes
: "${FALLBACK_OUT:=rtmp://localhost:1935/yt_fallback_b}"           # where producer_fallback_ytb publishes

OBS_PRODUCER="$HOME/producer_obs_ytb.sh"
FALLBACK_PRODUCER="$HOME/producer_fallback_ytb.sh"
CONSUMER="$HOME/mux_consumer_ytb.sh"

# Ensure scripts are executable
chmod +x "$OBS_PRODUCER" "$FALLBACK_PRODUCER" "$CONSUMER" || true

printf '[%(%a %b %d %T %Z %Y)T] Supervisor B starting. INPUT_URL=%s YT_INGEST=%s KEY=%s\n' -1 "$INPUT_URL" "$YT_INGEST" "${YT_KEY:+(set)}"
...
start_if_missing "producer_obs_ytb" "producer_obs_ytb.sh" "$OBS_PRODUCER"
start_if_missing "producer_fallback_ytb" "producer_fallback_ytb.sh" "$FALLBACK_PRODUCER"

# Consumer loop: pushes to YouTube using the current YT_KEY (foreground)
while true; do
  printf '[%(%a %b %d %T %Z %Y)T] Starting mux_consumer_ytb...\n' -1
  ...
  if ! stdbuf -oL -eL env YT_INGEST="$YT_INGEST" \
          ...
          "$CONSUMER"; then
    rc=$?
    printf '[%(%a %b %d %T %Z %Y)T] mux_consumer_ytb exited (rc=%s). Restarting in 3s...\n' -1 "$rc"
    sleep 3
  fi
done