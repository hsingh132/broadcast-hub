...
# "obs/ytb" app so the muxer can switch between OBS and fallback.
#
...
# It must ONLY push to the local RTMP endpoint rtmp://127.0.0.1:1935/obs/ytb.
# The mux_watch_ytb.sh script will do the A/B switching and the consumer
# will push to YouTube.
#
# Env vars (optional):
#   INPUT_URL   - RTMP URL where OBS is publishing (default: rtmp://localhost:1935/live/test)
#   OBS_APP_URL - Local RTMP endpoint that receives OBS for YT-B (default: rtmp://127.0.0.1:1935/obs/ytb)
...
OBS_APP_URL="${OBS_APP_URL:-rtmp://127.0.0.1:1935/obs/ytb}"

echo "==> producer_obs_ytb starting"
...
terminate() {
  echo "$(ts) [producer_obs_ytb] Caught signal, exiting..."
  exit 0
}
...
# Main retry loop
while true; do
  echo "$(ts) [producer_obs_ytb] Launching ffmpeg to pull OBS and push to $OBS_APP_URL"
  # Notes:
  ...
  status=$?
  echo "$(ts) [producer_obs_ytb] ffmpeg exited with status $status"
  # If clean exit, stop looping; if failure, retry shortly.
  if [[ $status -eq 0 ]]; then
    echo "$(ts) [producer_obs_ytb] Clean exit. Stopping."
    break
  fi
  echo "$(ts) [producer_obs_ytb] Will retry in 2s..."
  sleep 2
done